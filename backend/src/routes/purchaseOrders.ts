...
router.post("/", authenticate, requireRole("ADMIN"), async (req: Request, res: Response) => {
    try {
        ...
        let subtotal = 0;
        let taxAmount = 0;
        const processedItems = items.map((item: { productId: string; quantity: number; unitPrice: number; taxRate?: number; description?: string; notes?: string }) => {
...
router.patch("/:id/confirm", authenticate, requireRole("ADMIN"), async (req: Request, res: Response) => {
    try {
        const po = await prisma.purchaseOrder.findUnique({ where: { id: req.params.id } });
        if (!po) return res.status(404).json({ success: false, error: "PO not found" });
        if (po.status !== "DRAFT") return res.status(400).json({ success: false, error: "Only DRAFT POs can be confirmed" });
...
// ─── Receive Goods (GRN) ─────────────────────────────────────────────
router.post("/:id/receive", authenticate, requireRole("ADMIN"), async (req: Request, res: Response) => {
    try {
        const user = (req as unknown as { user: { userId: string } }).user;
...
// ─── Cancel PO ───────────────────────────────────────────────────────
router.patch("/:id/cancel", authenticate, requireRole("ADMIN"), async (req: Request, res: Response) => {
    try {
        const po = await prisma.purchaseOrder.findUnique({ where: { id: req.params.id } });
        if (!po) return res.status(404).json({ success: false, error: "PO not found" });
