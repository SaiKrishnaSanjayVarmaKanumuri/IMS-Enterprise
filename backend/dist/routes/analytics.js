...
// ─── Dashboard KPIs ──────────────────────────────────────────────────
router.get("/kpis", auth_js_1.authenticate, async (req, res) => {
    try {
        const { siteId } = req.query;
        const siteFilter = siteId ? { siteId: String(siteId) } : {};
        const [totalInventoryItems, lowStockCount, pendingRequests, pendingPOs, totalVendors, recentMovements, totalSites, activeSites,] = await Promise.all([
            ...
            prisma.inventoryItem.findMany({ where: siteFilter }).then(items => items.filter(i => i.minimumStock > 0 && i.currentStock <= i.minimumStock).length)),
...
router.get("/stock-trends", auth_js_1.authenticate, async (req, res) => {
    try {
        ...
        movements.forEach(m => {
            ...
            if (!byDate[date])
                byDate[date] = { date, added: 0, consumed: 0 };
            if (m.type === "ADD")
                byDate[date].added += m.quantity;
            else if (m.type === "CONSUME")
                byDate[date].consumed += m.quantity;
        });
...
// ─── Top Consumed Items ───────────────────────────────────────────────
router.get("/top-consumed", auth_js_1.authenticate, async (req, res) => {
    try {
...
router.get("/request-distribution", auth_js_1.authenticate, async (req, res) => {
    try {
        const { siteId } = req.query;
        const siteFilter = siteId ? { siteId: String(siteId) } : {};
...
// ─── Low Stock Items ──────────────────────────────────────────────────
router.get("/low-stock", auth_js_1.authenticate, async (req, res) => {
    try {
        const { siteId } = req.query;
        const siteFilter = siteId ? { siteId: String(siteId) } : {};
...
// ─── Inventory Valuation ─────────────────────────────────────────────
router.get("/inventory-valuation", auth_js_1.authenticate, async (req, res) => {
    try {
        const { siteId } = req.query;
        const siteFilter = siteId ? { siteId: String(siteId) } : {};
        const items = await prisma.inventoryItem.findMany({
        ...
        valuationData.forEach(i => {
            if (!byCategory[i.category])
                byCategory[i.category] = { category: i.category, totalValue: 0, itemCount: 0 };
...
// ─── Vendor Performance ───────────────────────────────────────────────
router.get("/vendor-performance", auth_js_1.authenticate, async (req, res) => {
    try {
        ...
        const performance = vendors.map(v => {
            ...
            const onTime = v.purchaseOrders.filter(po => po.receivedDate && po.expectedDate && po.receivedDate <= po.expectedDate).length;
