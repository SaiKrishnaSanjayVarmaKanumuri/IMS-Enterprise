 ...
 */
router.post("/", auth_js_1.authenticate, (0, rbac_js_1.requirePermission)("requests:create"), async (req, res) => {
    try {
        const user = req.user;
        const { type, siteId, targetSiteId, description, justification, expectedDate, priority, items, } = req.body;
        ...
        // Log audit
        (0, logger_js_1.logAudit)(user.id, "request.create", "requests", request.id, {
            requestNumber: request.requestNumber,
            type,
            siteId,
            itemCount: items.length,
        }, "success");
        logger_js_1.logger.info(`Request ${request.requestNumber} created by ${user.email}`);
        ...
        logger_js_1.logger.error("Create request error:", error);
        res.status(500).json({
            success: false,
 ...
 * - Admin: All requests
 */
router.get("/", auth_js_1.authenticate, (0, rbac_js_1.requirePermission)("requests:read:own"), async (req, res) => {
    try {
        const user = req.user;
        ...
        logger_js_1.logger.error("Get requests error:", error);
        res.status(500).json({
            success: false,
 ...
 */
router.get("/:id", auth_js_1.authenticate, (0, rbac_js_1.requirePermission)("requests:read:own"), async (req, res) => {
    try {
        const { id } = req.params;
        const user = req.user;
        ...
        if (!hasAccess) {
            (0, logger_js_1.logAudit)(user.id, "request.access_denied", "requests", id, { role: user.role }, "failed");
            res.status(403).json({
                success: false,
        ...
        logger_js_1.logger.error("Get request error:", error);
        res.status(500).json({
            success: false,
 ...
 */
router.patch("/:id/approve", auth_js_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const user = req.user;
        ...
        (0, logger_js_1.logAudit)(user.id, "request.approve", "requests", id, {
            requestNumber: request.requestNumber,
            fromStatus: request.status,
            toStatus: nextStatus,
            role,
        }, "success");
        logger_js_1.logger.info(`Request ${request.requestNumber} approved by ${user.email} (${role})`);
        ...
        logger_js_1.logger.error("Approve request error:", error);
        res.status(500).json({
            success: false,
 ...
 */
router.patch("/:id/reject", auth_js_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const user = req.user;
        const { remarks } = req.body;
        ...
        (0, logger_js_1.logAudit)(user.id, "request.reject", "requests", id, {
            requestNumber: request.requestNumber,
            fromStatus: request.status,
            toStatus: rejectedStatus,
            role,
        ...
        logger_js_1.logger.info(`Request ${request.requestNumber} rejected by ${user.email} (${role})`);
        res.json({
            success: true,
        ...
        logger_js_1.logger.error("Reject request error:", error);
        res.status(500).json({
            success: false,
 ...
 */
router.patch("/:id/cancel", auth_js_1.authenticate, (0, rbac_js_1.requirePermission)("requests:cancel"), async (req, res) => {
    try {
        const { id } = req.params;
        const user = req.user;
        ...
        (0, logger_js_1.logAudit)(user.id, "request.cancel", "requests", id, { requestNumber: request.requestNumber, reason }, "success");
        logger_js_1.logger.info(`Request ${request.requestNumber} cancelled by ${user.email}`);
        res.json({
            success: true,
        ...
        logger_js_1.logger.error("Cancel request error:", error);
        res.status(500).json({
            success: false,
 ...
 */
router.patch("/:id/procurement", auth_js_1.authenticate, (0, rbac_js_1.requirePermission)("procurement:assign-vendor"), async (req, res) => {
    try {
        const { id } = req.params;
        const user = req.user;
        const { vendorId, vendorName, poNumber, procurementNotes, actualCost, } = req.body;
        const request = await prisma.request.findUnique({
            where: { id },
        });
        if (!request) {
        ...
        (0, logger_js_1.logAudit)(user.id, "request.procurement_update", "requests", id, { vendorId, vendorName, poNumber }, "success");
        res.json({
            success: true,
            data: { request: updatedRequest },
        ...
        logger_js_1.logger.error("Procurement update error:", error);
        res.status(500).json({
            success: false,
